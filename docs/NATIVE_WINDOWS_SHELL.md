...
Development package `1.0.0.6` added the first creation/connection safeguards, and the subsequent native correction makes the folder-picker contract explicit. Any selected empty folder is now used exactly as the vault location, even when its folder name differs from the vault's display name. Selecting a non-empty parent creates the named child, while selecting a matching non-empty folder is rejected instead of producing `Name\\Name`. The wizard displays the exact final path before creation. The create protocol transmits the user-entered display name separately from that path, and the engine validates and applies it immediately after registration; selecting `F:\\MooseTaxes` for a vault named `MyMoose` therefore shows `MyMoose` at `F:\\MooseTaxes`. The validated name and exact path are frozen when Review opens, so the final Create action does not reread hidden Step 1 controls. Engine validation reports invalid paths, names, and passwords independently instead of collapsing them into a generic request error. Connect-vault validation errors wrap within the available content width. The package builder also signs and verifies the authored EXE and DLL before packing a development MSIX, then signs and verifies the package envelope. This was added after Windows Application Control blocked version `1.0.0.3` when a refreshed stage contained unsigned internal binaries even though its MSIX envelope was valid.
