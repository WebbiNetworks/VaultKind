...
Development MSIX packages are required to contain a signed-content `VaultKind.PackageProfile.json` marker generated by `build-native-msix.ps1`. Its validated identifier moves only that package's engine settings, logs, plugins, mount-point base, and local socket beneath `%LOCALAPPDATA%\VaultKind\PackageProfiles\<id>`; the Java child receives the same root through its process-local `LOCALAPPDATA`. The native client and lifecycle host share one resolver, so a development package cannot attach to the permanent unpackaged engine or display its vault list. Missing markers preserve the ordinary permanent profile for unpackaged builds. Malformed, non-development, path-traversing, or stale pre-staged markers fail closed, and production packaging rejects the development-only profile option.

The July 31 development package inspection confirmed that this marker is present only inside the signed `.Development` MSIX and names `LocalMsix.WebDav`; the ordinary staged release remains marker-free. The package has not yet been installed, so installed identity resolution, isolated engine startup, explicit Windows Explorer WebDAV selection, and disposable mounted-file I/O remain separate runtime gates.
