...
Class-level trimming is fail-closed and incremental. The first thirty-five physical slices include exact partial slices of `org.cryptomator.ui.unlock`, `org.cryptomator.ui.health`, `org.cryptomator.ui.mainwindow`, `org.cryptomator.ui.fxapp`, `org.cryptomator.launcher`, `org.cryptomator.ui.common`, `org.cryptomator.networking`, `org.cryptomator.nativeui`, `org.cryptomator.event`, `org.cryptomator.logging`, the direct `org.cryptomator.common` package, and `org.cryptomator.common.settings`, plus the exact retired `org.cryptomator.ipc` package and test-only root `JavaFXUtil`; every compiled package or partial-package union must still match its explicit reviewed set before anything is omitted. The launcher slice removes 131 retired JavaFX/IPC entry and generated-Dagger classes while retaining the exact twenty-class native/shared launcher closure. The common UI slice removes only four presentation helpers while retaining its other thirty-six workflow and compatibility classes. The Windows-only networking slice removes four macOS/Linux trust-store classes while retaining the seven-class Windows TLS/provider family for future Hub HTTPS compatibility. The separate nine-class legacy IPC stack is removed only after proving it has no staged inbound edge and is unrelated to the retained native socket bridge. The test-only JavaFX platform-startup helper remains in source but not the native package. Nine unused generated native factories are omitted while all twenty-one functional native classes remain; four unused generated event factories are omitted while all nine functional event classes remain; the unused generated debug-mode factory is omitted while all six functional logging classes remain; seven unused generated direct-common wrappers are omitted while functional licensing, executor creation, reveal-path provider discovery, and thirty-one direct common classes remain; three unused generated settings factories are omitted while all sixteen functional settings classes remain. The July 29 filtered stage retains all 145 statically reachable authored classes, passes all 67 native checks and the complete disposable-vault protocol probe through `NativeBackendMain`, and has decreased by exactly 2,158,318 bytes to 277,704,403 bytes. Reachable event aggregation, notification interfaces, reveal-path integrations, encrypted-vault health/repair behavior, functional native and legacy settings, startup, diagnostics/logging, key-loading, keychain, vault unlock, mount-provider services, all seventy Hub key-loading classes, all seven common keychain classes, the Windows TLS provider, functional licensing, neutral vault creation and recovery primitives, and all twenty-five vault-format migration classes remain packaged. Migration and the dynamically loaded tray-menu provider remain protected. Broad wildcard removal and third-party JAR filtering remain prohibited.
