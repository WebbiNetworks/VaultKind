...
Class-level trimming is fail-closed and incremental. The first twenty-six physical slices include exact partial slices of `org.cryptomator.ui.unlock`, `org.cryptomator.ui.health`, `org.cryptomator.ui.mainwindow`, `org.cryptomator.ui.fxapp`, `org.cryptomator.launcher`, and `org.cryptomator.ui.common`; every compiled package or partial-package union must still match its explicit reviewed set before anything is omitted. The launcher slice removes 131 retired JavaFX/IPC entry and generated-Dagger classes while retaining the exact twenty-class native/shared launcher closure. The common slice removes only four presentation helpers while retaining its other thirty-six workflow and compatibility classes. The July 29 filtered stage retains all 145 statically reachable authored classes, passes all 67 native checks and the complete disposable-vault protocol probe through `NativeBackendMain`, and has decreased by exactly 2,070,585 bytes to 277,792,136 bytes. Reachable event aggregation, notification interfaces, reveal-path integrations, encrypted-vault health/repair behavior, neutral settings, startup, diagnostics/logging, key-loading, keychain, vault unlock, mount-provider services, all seventy Hub key-loading classes, all seven common keychain classes, the Windows provider, neutral vault creation and recovery primitives, and all twenty-five vault-format migration classes remain packaged. Migration and the dynamically loaded tray-menu provider remain protected. Broad wildcard removal and third-party JAR filtering remain prohibited.
