...
Class-level trimming is fail-closed and incremental. The first ten physical slices name every reviewed class in `org.cryptomator.ui.dialogs`, `org.cryptomator.ui.wrongfilealert`, `org.cryptomator.ui.updatereminder`, `org.cryptomator.ui.sharevault`, `org.cryptomator.ui.stats`, `org.cryptomator.ui.decryptname`, `org.cryptomator.ui.error`, `org.cryptomator.ui.controls`, `org.cryptomator.ui.quit`, and `org.cryptomator.ui.lock`; each compiled package must still match its explicit reviewed set before it is omitted. The July 29 filtered stage retained all 145 statically reachable authored classes, passed the complete zero-vault protocol probe including mount-provider inventory and graceful shutdown, and decreased by exactly 346,762 bytes to 279,515,959 bytes. The tray-menu package remains because its AWT implementation is a dynamically loaded service provider. Broad wildcard removal and third-party JAR filtering remain prohibited.
