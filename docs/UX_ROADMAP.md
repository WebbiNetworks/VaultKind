...
- The isolated Store-path proof is complete: Microsoft-certified versions `1.0.0.0` and `1.0.1.0` passed acquisition, supported app-volume relocation, Store-managed update, uninstall, clean reinstall, and launch after both update and reinstall. The real Microsoft-signed VaultKind `1.0.0.0` package has now passed acquisition, isolated-profile launch, exact-folder disposable-vault creation, Windows Explorer WebDAV file I/O, lock/unmount, close/relaunch, uninstall/reinstall, external encrypted-data preservation, reconnect, persisted-file recovery, and supported relocation to `G:\WindowsApps` with post-move launch/unlock/read/lock verification. A local `1.0.1.0` candidate carrying the corrected conditional recovery warning has passed package inspection, all 87 native checks, and a full overall-PASS WACK run. The final clean artifact must still be rebuilt from the committed correction and delivered through Microsoft Store to prove the real-product update before public discoverability.
