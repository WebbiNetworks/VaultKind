...
- The isolated Store-path proof is complete: Microsoft-certified versions `1.0.0.0` and `1.0.1.0` passed acquisition, supported app-volume relocation, Store-managed update, uninstall, clean reinstall, and launch after both update and reinstall. The real Microsoft-signed VaultKind `1.0.0.0` package passed acquisition, isolated-profile launch, exact-folder disposable-vault creation, Windows Explorer WebDAV file I/O, lock/unmount, close/relaunch, uninstall/reinstall, external encrypted-data preservation, reconnect, persisted-file recovery, and supported relocation to `G:\WindowsApps` with post-move launch/unlock/read/lock verification. The certified real-product `1.0.1.0` update carrying the corrected conditional recovery warning then installed through Microsoft Store, launched from its exact `1.0.1.0` package path, retained the external validation vault, and closed normally. Submission 3 subsequently completed the separately approved public-discoverability launch. All Store-specific engineering validation gates are complete.
