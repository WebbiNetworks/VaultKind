...
The first operation is `backend.hello`. It proves the expected backend identity, correlates the request identifier, and negotiates protocol version 1 before any vault information is requested. Once it succeeds on the same connection, `vault.list` returns only the privacy-limited summaries defined above. Both request and response frames use a four-byte big-endian length followed by bounded UTF-8 JSON, with a 64 KiB maximum.
